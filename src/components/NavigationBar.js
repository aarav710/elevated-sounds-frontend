  ...
  return (
    <nav>
      <ul className="components">
        ...
        <div className="brand">
          <img src={'https://infinite-crag-03585.herokuapp.com/elevated-sounds-logo.png'} alt="logo" width="50px" height="50px" className="logo" />
        ...
        <div className={isOpen ? 'pages-open pages' : 'pages-closed pages'}>
          ...
          {user.ID !== null ?
            <Link to="/" className="navStyle" onClick={() => { updateUser({ type: 'LOG OUT' }); fetch('https://infinite-crag-03585.herokuapp.com/logout', { credentials: 'include', headers: { 'Content-Type': 'application/json', }, }); setOpen(false); history.push("/");}} key={6}>

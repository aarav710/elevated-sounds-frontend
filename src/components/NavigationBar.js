  ...
  return (
    <nav>
      <ul className="components">
        ...
        <div className="brand">
          <img src={`${REACT_APP_BACKEND_URL}/elevated-sounds-logo.png`} alt="logo" width="50px" height="50px" className="logo" />
        ...
        <div className={isOpen ? 'pages-open pages' : 'pages-closed pages'}>
          ...
          {user.ID !== null ?
            <Link to="/" className="navStyle" onClick={() => { updateUser({ type: 'LOG OUT' }); fetch(`${REACT_APP_BACKEND_URL}/logout`, { credentials: 'include', headers: { 'Content-Type': 'application/json', }, }); setOpen(false); history.push("/");}} key={6}>
